...
Page({
  data: {
    ...
    // 订单抽屉
    orderActiveTab: 'ALL',
    allOrders: [] as OrderModel[],
    filteredOrders: [] as OrderModel[],

    // 优惠券抽屉（营销接口接通前不注入虚构优惠券）
    couponActiveTab: 'available',
    availableCoupons: [] as any[],
    expiredCoupons: [] as any[],
  ...
  async syncUserData() {
    try {
      ...
      const list = rawList.map(o => ({
        ...o,
        payAmountYuan: (Number(o.payAmount || 0) / 100).toFixed(2),
        items: (o.items || []).map(i => ({
          ...i,
      ...
      try {
        const cloudAddresses = await AddressService.list();
  ...
  onTapOrder(e: any) {
    ...
    let targetTab = 'ALL';
    if (rawKey === 'unpaid') targetTab = 'PENDING_PAYMENT';
    else if (rawKey === 'unshipped') targetTab = 'PAID';
    else if (rawKey === 'shipped') targetTab = 'SHIPPED';
    else if (rawKey === 'refund') targetTab = 'REFUND';

    wx.navigateTo({ url: `/pages/order/list/index${targetTab === 'ALL' ? '' : `?status=${targetTab}`}` });
  },

  onSelectOrderTab(e: any) {
  ...
  async onPayOrder(e: any) {
    ...
    try {
      ...
      if (payRes && payRes.status === 'PAID') {
        ...
        wx.showToast({ title: payRes?.message || '支付处理中，请稍后刷新', icon: 'none' });
      }
  ...
  // 收藏交互
  // -------------------------
  async onAddToCartFromFav(e: any) {
    const item = e.currentTarget.dataset.item;
    try {
      await CartService.addToCart(`sku_${item.id}_42`, 1, {
        productId: item.id,
        title: item.title,
        price: item.price,
        image: item.cover,
  ...
  // 收货地址交互
  // -------------------------
  fromCloudAddress(item: CloudAddress): UserAddress {
    return {
  ...
  toCloudAddress(item: UserAddress, isDefault = item.isDefault): Omit<CloudAddress, 'id'> & { id?: string } {
  ...
  async onSetDefaultAddress(e: any) {
    const id = e.currentTarget.dataset.id;
    const current = this.data.addresses.find(a => a.id === id);
    if (!current) return;
    try {
      await AddressService.save(this.toCloudAddress(current, true));
      await this.syncUserData();
  ...
  async onSaveAddress() {
    ...
    try {
      const saved = await AddressService.save(this.toCloudAddress(newAddr, newAddr.isDefault));
